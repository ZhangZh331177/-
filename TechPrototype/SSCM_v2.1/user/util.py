...
def check_login(func):
    ...
    def _check(*args, **kwargs):
        ...
        if cookie_kind not in ["student", "teacher", "manager"]:
            # Not logged in
...
def get_user(request, kind):
    ...
    """
    if request.session.get('kind', '') != kind or kind not in ["student", "teacher", "manager"]:
        return None
    ...
    if kind == "student":
        ...
        # grade = uid[:4]
        number = uid # [4:]
        student_set = Student.objects.filter(number=number) # (grade=grade, number=number)
        ...
        # 找到对应管理员
        number = uid # [3:]
        manager_set = Manager.objects.filter(number=number)
        if manager_set.count() == 0:
            return None
        return manager_set[0]

    else:
        # 找到对应老师
        # department_no = uid[:3]
        number = uid # [3:]
        teacher_set = Teacher.objects.filter(number=number) # (department_no=department_no, number=number)
