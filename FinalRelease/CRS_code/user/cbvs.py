...
from user.forms import StuRegisterForm, TeaRegisterForm, ManRegisterForm
...
class CreateTeacherView(CreateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            return self.form_invalid(form)
    def form_valid(self, form):
        # 【已废弃规则】老师注册时随机生成院系号, 院系号范围为[0,300)
...
class CreateManagerView(CreateView):
    ...
    def form_valid(self, form):
        ...
        uid = new_manager.number
        from_url = "register"
        base_url = reverse(self.get_success_url(), kwargs={'kind': 'manager'})
        return redirect(base_url + '?uid=%s&from_url=%s' % (uid, from_url))
    def get_context_data(self, **kwargs):
...
class UpdateManagerView(UpdateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        return context
    def get_success_url(self):
        return reverse("course", kwargs={"kind": "manager"})
