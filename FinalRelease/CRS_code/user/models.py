...
def validate_stu_length(value):
    if (value<1e11) or (value>=1e12):
        raise ValidationError('学号应为非0开头的12位数字')
def validate_length(value):
    if (value<1e4)or (value>=1e5):
        raise ValidationError('教职工号的长度应为非0开头的5位数字')
...
class Student(models.Model):
    ...
    number = models.BigIntegerField(verbose_name="学号", primary_key=True, validators=[validate_stu_length])
...
class Teacher(models.Model):
    ...
    name = models.CharField(max_length=50, verbose_name="姓名")
    gender = models.CharField(max_length=10, choices=genders, default='m', verbose_name="性别")
    number = models.BigIntegerField(verbose_name="教职工号", primary_key=True, validators=[validate_length])
    email = models.EmailField(max_length=254,verbose_name="邮箱")
    password = models.CharField(max_length=30, verbose_name="密码", validators=[validate_pas_length])
    # info = models.CharField(max_length=255, verbose_name="个人简介", null=True, default='无', help_text="自我介绍，不超过250字")

    def get_teacher_list():
        # 获取dynamic_field的所有可能值
        return [(item.number, f"{item.name} ({item.number})") for item in Teacher.objects.all()]
