...
class ScoreUpdateView(UpdateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if self.object:
            ...
            return_url = reverse("view_detail_tea", kwargs={"course_id": self.object.course.id})

        return self.render_to_response(self.get_context_data(info=info, title=title, return_url=return_url))

    def get_success_url(self):
        if self.object:
            return reverse("view_detail_tea", kwargs={"course_id": self.object.course.id})
        else:
...
class RateUpdateView(UpdateView):
    ...
    def get_success_url(self):
        return reverse("view_course", kwargs={"view_kind": "current"})
